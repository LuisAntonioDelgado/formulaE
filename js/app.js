...
const changeImage = (toAction) =>{
    ...
    let newImage = document.getElementById("auto1");

    if(toAction === "running"){

        newImage.setAttribute("src", "./img/autos/auto1v.png");

    }else if(toAction === "idle"){

        newImage.setAttribute("src", "./img/autos/auto1.png");
...
function animate() {
    ...
    if (modelReady){
        ...
        if(bndReset2){
            bndReset2 = false;
            // console.log(bndReset2);
            runner.position.z = 5;
            targetCamera = { x: 0, y: 10, z: -10 };
            positionCamera = { x: 10, y: 20, z: 20 }
            logIni = 0;
            animations[pj].idle();
        }
        updateCamera()
...
function checkCollisions(position){
    let ini = position + 0.5;
    let fin = position - 0.5;
